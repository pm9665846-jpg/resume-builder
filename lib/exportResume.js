export async function exportToPDF(elementId) {
  ...
  // ── 1. Save original styles of element AND all its ancestors ──
  const ancestors = []
  let node = element
  ...
  try {
    // ── 2. Force 794px on element and remove overflow clipping on all ancestors ──
    ...
    // Set element itself to full width
    ...
    const canvas = await html2canvas(element, {
      ...
      y: 0,
      ignoreElements: (el) => {
        // Ignore any overlay/modal elements
        const id = el.id || ''
        return id === 'print-overlay'
      },
    })

    // ── 5. Restore all original styles ──
    ancestors.forEach(({ el, overflow, overflowX, overflowY, width, maxWidth, minWidth, transform, transformOrigin, height, minHeight, position }) => {
