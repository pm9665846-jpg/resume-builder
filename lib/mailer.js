...
export async function sendPasswordResetOTP(email, otp, name) {
  ...
  if (!emailPass || emailPass === 'your_email_app_password') {
    console.log(`\n========================================`)
  ...
  await transporter.sendMail({
    from: `"Fresh CV" <${process.env.EMAIL_FROM}>`,
    to: email,
    subject: `${otp} — Your Fresh CV Password Reset Code`,
    html,
    text: `Your Fresh CV password reset OTP is: ${otp}\n\nThis code expires in 15 minutes.\n\nIf you didn't request this, ignore this email.`,
  })
  } catch (emailErr) {
    ...
    console.log(`📧 OTP for ${email}: ${otp}`)
    console.log(`========================================\n`)
    // Don't throw — OTP saved in DB, user can still use it
  }
}
